...
import pandas as pd
from shapely.geometry import Point, MultiLineString, MultiPolygon, MultiPoint
from shapely import ops
from shapely.wkt import loads
from shapely import prepared
from .digiroad_time_penalties import penalties as pns
from .graphio import build_igraph_from_Digiroad

def process_speed_limits(link_gdf, limit_gdf, road_type_col='TOIMINN_LK', known_speed_limit_col='ARVO', speed_limit_col='KmH', length_col='Pituus', freeflow_col='Digiroa_aa'):
...
def is_deadend(nodeid, neighbors):
    """
    Find out whether given node is dead-end.
    Dead-end is determined as an endpoint who has only 1 neighbor.
    """
    # Remove itself from the neighbors
    if nodeid in neighbors:
        neighbors.remove(nodeid)
    # If there is only 1 neighbor for the node, it must be a dead-end
    if len(neighbors) == 1:
        return True
    else:
        return False
...
def is_endpoint(nodeid, neighbors, neighbor_cnt, degree, indegree, outdegree):
    """
    Find out whether given node is an endpoint of a link.
    Follows the same principles as determined in osmnx.is_endpoint().
    """
    if nodeid in neighbors:
        # if the node appears in its list of neighbors, it self-loops. this is
        # always an endpoint.
        return True
    # if node has no incoming edges or no outgoing edges, it must be an endpoint
    elif outdegree == 0 or indegree == 0:
        return True
    elif not (neighbor_cnt == 2 and (degree == 2 or degree == 4)):
        # else, if it does NOT have 2 neighbors AND either 2 or 4 directed
        # edges, it is an endpoint. either it has 1 or 3+ neighbors, in which
        # case it is a dead-end or an intersection of multiple streets or it has
        # 2 neighbors but 3 degree (indicating a change from oneway to twoway)
...
def get_nodes_gdf_from_igraph(G, geometry='geometry', xcol='x', ycol='y'):
    """Get nodes GeoDataFrame from graph"""
    geo = gpd.GeoDataFrame()
    # Attributes in nodes
    cols = G.vs.attributes()
    for col in cols:
        geo[col] = G.vs[col]
    # Make geometry
    if geometry in geo.columns:
        ...
        if xcol in geo.columns and ycol in geo.columns:
            geo[geometry] = gpd.points_from_xy(geo[xcol], geo[ycol])
        else:
            raise ValueError("Could not create geometry. Missing coordinate information.")
    return geo


def get_intersections_from_igraph(G, node_id_col='node_id'):
    """
    Get intersections from igraph.
    """
    # Get nodes of the graph
    nodes = get_nodes_gdf_from_igraph(G)

    # Mask for results
    results = []

    for node in nodes.itertuples():
        node_attr = dict(node._asdict())
        nodeid = node_attr[node_id_col]
        # Get neighbors
        neighbors = list(set(G.neighbors(nodeid)))
        n = len(neighbors)
        d = G.degree(nodeid)
        in_d = G.indegree(nodeid)
        out_d = G.outdegree(nodeid)

        # Check if node is an endpoint of link
        if is_endpoint(nodeid, neighbors, n, d, in_d, out_d):
            # If endpoint is NOT a dead-end, it must be an intersection
            if not is_deadend(nodeid, neighbors):
                results.append(True)
            else:
                results.append(False)
        # If the node is not an endpoint of the link, it cannot be intersection either.
        else:
            results.append(False)
    nodes['is_intersection'] = results
    # Return nodes that are intersections
    return nodes.loc[nodes['is_intersection'] == True].copy()
...
def apply_intersection_delays(links, limits, signals):
    """
    ...
    data = process_speed_limits(link_gdf=links, limit_gdf=limits,
    ...
    # Generate igraph from the roads
    G = build_igraph_from_Digiroad(data)

    # Parse intersections from the graph
    intersections = get_intersections_from_igraph(G)
    ...
    # Find out which intersections are affected by traffic signals
